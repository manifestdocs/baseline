...
fn check_function(node: Node, source: &str, file: &str, diagnostics: &mut Vec<Diagnostic>) {
    ...
    // Find the function body (the expression after `=`)
    let body = find_function_body(node);

    if let Some(body_node) = body {
        // Walk the body looking for effectful calls
...
fn check_node_for_effects(
    ...
    if node.kind() == "effectful_identifier" {
    ...
    // Check for qualified effectful calls like Http.get!
