...
Rocket is designed around three core principles:
...
Rocket is designed to be generated and analyzed by Large Language Models. This influences the language design in several ways:

#### 1.5.1 Constrained Solution Space

Traditional languages allow virtually unlimited ways to accomplish a task. Rocket constrains the solution space through:
...
LLMs cannot step through debuggers or intuit runtime behavior. Rocket provides compile-time verification:
...
This makes Rocket suitable for **untrusted AI-generated code** execution.
...
Rocket is opinionated by design. For every common operation, there is one obvious way to do it. This is not merely a convention—it is enforced by the compiler and formatter.
...
// NOT supported - these don't exist in Rocket:
...
`rocket fmt` enforces the one true style. **It has no configuration options.**
...
`rocket lint` warns on non-idiomatic patterns that are syntactically valid but discouraged:

```bash
$ rocket lint src/

src/api.rocket:12:5 warning[W001]: use .map() instead of accumulator loop
   |
12 |   for x in items do
   |   ^^^ consider: items.map(|x| transform(x))

src/api.rocket:45:1 warning[W002]: prefer explicit imports
   |
45 | import Http.*
   | ^^^^^^^^^^^^^ list specific imports: import Http.{get!, post!}

src/api.rocket:67:5 warning[W003]: use pipe for transformation chain
   |  
...
| Formatting | Always `rocket fmt` canonical output |
| Imports | Always explicit (wildcard warns) |

**The result**: Any Rocket codebase looks like any other Rocket codebase. LLMs trained on Rocket code see consistent patterns. Humans reading Rocket code have no style surprises.

### 1.7 Core Design Principles

Beyond syntax and tooling, Rocket embodies two fundamental design principles that shape how programs are structured.

#### 1.7.1 Composition Over Inheritance

Rocket has no classes, no inheritance hierarchy, no `extends`, no `super`, no `instanceof`. These concepts do not exist in the language.
...
**Instead of inheritance, Rocket provides:**
...
These principles are not suggestions—they are enabled and enforced by Rocket's type system. Inheritance is impossible. Unvalidated data at internal boundaries is a type error. Invalid states are unrepresentable. The compiler is your partner in maintaining these invariants.
...
Rocket source files are UTF-8 encoded. The file extension is `.rocket`.
...
Rocket's syntax is designed to minimize cascading changes when code is modified, which is critical for LLM-generated patches and version control:
...
When using `matches()` in refinement types, Rocket uses a **deterministic regex subset** to ensure consistent behavior between compile-time checking and runtime validation:
...
Rocket uses bidirectional type inference. Type annotations are required at:
...
To reduce boilerplate for scripts and simple programs, Rocket provides a **standard prelude** with default effect handlers:

```rocket
// WITHOUT prelude: explicit handler setup
module MyScript

import Rocket.Effects.*
...
```
my-project/
├── rocket.toml         // Project configuration
├── src/
│   ├── main.rocket     // Entry point
│   ├── lib.rocket      // Library root (optional)
│   ├── api/
│   │   ├── mod.rocket  // Api module
│   │   ├── users.rocket
│   │   └── posts.rocket
│   └── util/
│       └── helpers.rocket
...
For web APIs, Rocket provides first-class API specs:
...
Rocket supports multiple verification levels to balance thoroughness with speed:
...
rocket check                    # Uses level from source annotations
rocket check --level=types      # Fast check only
rocket check --level=full       # Full verification
rocket check --timeout=30s      # Set SMT timeout
```
...
Checking specifications...
  ? process_data: verification timeout after 10s
    ...
    Options:
      1. Increase timeout: rocket check --timeout=60s
...
Rocket provides a comprehensive testing framework inspired by RSpec and Vitest, with first-class support for BDD-style specifications, property-based testing, and inline unit tests.

### 9.1 Testing Philosophy

Rocket testing follows three principles:
...
my-project/
├── src/
│   ├── user.rocket         // Contains inline tests
│   └── api.rocket          // Contains inline tests
└── test/
    ├── user_spec.rocket    // BDD specs for user module
    ├── api_spec.rocket     // BDD specs for api module
    ├── integration/
    │   └── full_flow_spec.rocket
    └── fixtures/
        └── test_data.rocket
```
...
```bash
$ rocket test --filter "UserService"
$ rocket test --filter "create_user"
$ rocket test user_spec.rocket
$ rocket test --tag integration
$ rocket test --tag "not slow"
```
...
[12:34:56] Running tests affected by src/user.rocket...
  ✓ 4 tests passed (23ms)

[12:35:12] Running tests affected by src/api.rocket...
  ✗ 1 test failed
...
Uncovered:
  src/api.rocket:47-52 (error handler branch)
  src/db.rocket:89 (connection retry logic)
  ...
  "failures": [
    {
      ...
      "location": "test/user_spec.rocket:45",
      "given": { "user": "bob", "post": "alice_post" },
      "when": "delete_post",
      "expected": "Err(Unauthorized)",
      "actual": "Err(NotFound)",
...
| Aspect | Rust Borrow Checker | Rocket Regions |
|--------|---------------------|----------------|
...
Rocket achieves memory safety through a simpler model:
...
Rocket implements **Perceus-style reference counting with FBIP optimization**, enabling purely functional code to execute with in-place mutation semantics when data is uniquely owned.
...
Source (.rocket files)
    ...
    Specification Verification (SMT)
        ↓
    Rocket IR (verified intermediate representation)
        ↓
    Optimization
        ↓
    ┌───────────────────────────────────┐
...
# Cross-compilation
rocket build --target=linux-arm64
rocket build --target=macos-x64
rocket build --target=windows-x64
```

### 11.3 Incremental Compilation

Rocket supports fine-grained incremental compilation:

```bash
$ rocket build
Compiling src/main.rocket... done (145ms)

# Edit src/api/users.rocket

$ rocket build
Recompiling src/api/users.rocket... done (23ms)
Linking... done (12ms)
```
...
Rocket can be embedded in other languages:

```c
// C/C++ embedding
#include "rocket.h"

rocket_runtime_t* rt = rocket_init();
rocket_module_t* mod = rocket_load(rt, "myapp.wasm");
rocket_value_t result = rocket_call(mod, "greet", "World");
printf("%s\n", rocket_to_string(result));  // "Hello, World"
rocket_free(rt);
```
...
Rocket programs can emit structured traces:
...
```json
{
  "events": [
    {
      "timestamp_ns": 142000,
      "location": { "file": "api.rocket", "line": 47, "col": 12 },
      "event": {
        "type": "function_call",
        "name": "get_user",
        "args": { "id": 42 }
    ...
    {
      "timestamp_ns": 891000,
      "location": { "file": "api.rocket", "line": 48, "col": 5 },
      "event": {
...
```bash
$ rocket trace analyze myapp.trace
...
Anomalies:
  ⚠ Db.query at api.rocket:48 took 2.3s (expected <100ms)
    Hypothesis: missing index on users.email
    
  ⚠ Unexpected None at api.rocket:52
...
```bash
$ rocket trace replay myapp.trace --until "api.rocket:52"

State at api.rocket:52:
  locals:
    id: 42
    user: None
...
$ rocket trace replay myapp.trace --query "why is user None?"
...
```bash
$ rocket trace replay myapp.trace --patch fix.rocket --dry-run

Original outcome: Error("unwrap on None at api.rocket:52")
...
```bash
rocket trace <file>             # Analyze trace file
rocket trace replay <file>      # Replay execution
rocket trace diff <a> <b>       # Compare two traces
rocket trace bisect <files...>  # Find regression
rocket trace query <file> "..." # Ask questions
rocket trace export <file>      # Export to JSON/Chrome trace
```
...
Rocket exposes its compiler internals via a queryable API, enabling IDEs, LLMs, and automated tools to interact with the language semantically. This goes beyond traditional LSP to support AI-assisted development.

### 13.2 Standard LSP Features

Rocket implements the full Language Server Protocol:
...
- **Formatting**: rocket fmt integration

### 13.3 Extended Query API

Beyond standard LSP, Rocket provides semantic queries:
...
{
  "method": "rocket/availableEffects",
  "params": {
    "file": "src/api.rocket",
...
```json
// Start a session
{ "method": "rocket/session/start", "params": { "project": "myapp" } }
// Response: { "sessionId": "abc123" }

// Submit code for checking
{
  "method": "rocket/session/check",
  "params": {
    "sessionId": "abc123",
    "file": "src/api.rocket",
    "source": "..."
...
// End session
{ "method": "rocket/session/end", "params": { "sessionId": "abc123" } }
```
...
For LLM repair loops and CI/CD integration, Rocket supports **SARIF (Static Analysis Results Interchange Format)** output—the industry standard for machine-parseable diagnostics.
...
SARIF is supported by GitHub, VS Code, and major CI systems. Using SARIF means Rocket diagnostics integrate automatically with existing toolchains.
...
rocket check src/ --format=sarif > results.sarif

# JSON output (Rocket's native format)
rocket check src/ --format=json
```
    ...
    "tool": {
      "driver": {
        ...
        "rules": [{
          ...
          "helpUri": "https://rocket-lang.dev/errors/REF_001"
        }]
      }
    },
    "results": [{
      ...
      "locations": [{
        "physicalLocation": {
          "artifactLocation": { "uri": "src/server.rocket" },
          "region": {
            "startLine": 42,
            "startColumn": 15,
            "endColumn": 19,
      ...
      "relatedLocations": [{
        "physicalLocation": {
          "artifactLocation": { "uri": "src/types.rocket" },
          "region": { "startLine": 5 }
        },
...
For advanced LLM integration, Rocket exposes type-checker state to enable **type-constrained token generation**—guiding LLM sampling toward well-typed code.
...
{
  "method": "rocket/completionConstraints",
  "params": {
...
Rocket modules export a standard interface:
...
(module
  ...
  (import "rocket" "effect_http_get" (func $http_get (param i32 i32) (result i32)))
  (import "rocket" "effect_console_print" (func $console_print (param i32 i32)))
  ;; ... etc
)
```
...
Rocket provides structured concurrency with lightweight fibers, channels, and a work-stealing scheduler.
...
Strategy and tooling for bootstrapping LLM training data for Rocket code generation.
...
The recommended pipeline for Rocket training data:

```
┌─────────────────────────────────────────────────────────────────┐
│  Phase 1: Seed Corpus (100-1000 examples)                       │
│  Hand-written, verified Rocket code covering core patterns      │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
...
│  Generate instruction prompts from existing Rocket snippets     │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
...
```bash
# Translate TypeScript to Rocket (best-effort)
rocket synth translate src/api.ts
# Output: src/api.rocket (may need manual review)

# Generate instruction prompt for existing code
rocket synth instruct src/validated.rocket
...
Rocket's syntax is designed to be translatable from well-supported languages:

| Source | Target Pattern | Confidence |
|--------|----------------|------------|
| TypeScript interfaces | Rocket records | High |
| TypeScript functions | Rocket functions | High |
| Rust Result/Option | Rocket Result/Option | Very High |
| Rust pattern matching | Rocket pattern matching | Very High |
| Python type hints | Rocket types | Medium |
| Go error handling | Rocket Result + ? | Medium |
