...
fn expect_string<'a>(name: &str, val: &RuntimeValue<'a>, pos: usize) -> Result<String, String> {
    match val {
        ...
        other => Err(format!("{} expects String for argument {}, got {}", name, pos, other)),
    }
}

fn string_length<'a>(args: &[RuntimeValue<'a>]) -> Result<RuntimeValue<'a>, String> {
    if args.len() != 1 {
        return Err(format!("String.length expects 1 argument, got {}", args.len()));
    }
...
fn string_starts_with<'a>(args: &[RuntimeValue<'a>]) -> Result<RuntimeValue<'a>, String> {
    if args.len() != 2 {
        return Err(format!("String.starts_with expects 2 arguments, got {}", args.len()));
    }
...
fn string_to_upper<'a>(args: &[RuntimeValue<'a>]) -> Result<RuntimeValue<'a>, String> {
    if args.len() != 1 {
        return Err(format!("String.to_upper expects 1 argument, got {}", args.len()));
    }
...
fn string_to_lower<'a>(args: &[RuntimeValue<'a>]) -> Result<RuntimeValue<'a>, String> {
    if args.len() != 1 {
        return Err(format!("String.to_lower expects 1 argument, got {}", args.len()));
    }
...
fn string_join<'a>(args: &[RuntimeValue<'a>]) -> Result<RuntimeValue<'a>, String> {
    ...
    let list = match &args[0] {
        ...
        other => return Err(format!("String.join expects List as first argument, got {}", other)),
    };
    ...
    let strs: Result<Vec<String>, String> = list
        ...
        .map(|item| match item {
            RuntimeValue::String(s) => Ok(s.clone()),
            other => Err(format!("String.join list elements must be Strings, got {}", other)),
        })
        .collect();
...
fn string_slice<'a>(args: &[RuntimeValue<'a>]) -> Result<RuntimeValue<'a>, String> {
    if args.len() != 3 {
        return Err(format!("String.slice expects 3 arguments, got {}", args.len()));
    }
    ...
    if start > s.len() || end > s.len() || start > end {
        return Err(format!("String.slice index out of bounds: {}..{} on string of length {}", start, end, s.len()));
    }
...
mod tests {
    ...
    fn trim() {
        ...
        assert_eq!(string_trim(&args).unwrap(), RuntimeValue::String("hi".into()));
    }

    #[test]
    fn contains_true() {
        let args = vec![RuntimeValue::String("hello".into()), RuntimeValue::String("ell".into())];
        assert_eq!(string_contains(&args).unwrap(), RuntimeValue::Bool(true));
    }

    #[test]
    fn contains_false() {
        let args = vec![RuntimeValue::String("hello".into()), RuntimeValue::String("xyz".into())];
        assert_eq!(string_contains(&args).unwrap(), RuntimeValue::Bool(false));
    }

    #[test]
    fn starts_with_true() {
        let args = vec![RuntimeValue::String("hello".into()), RuntimeValue::String("he".into())];
    ...
    fn to_upper() {
        let args = vec![RuntimeValue::String("hi".into())];
        assert_eq!(string_to_upper(&args).unwrap(), RuntimeValue::String("HI".into()));
    }

    #[test]
    fn to_lower() {
        let args = vec![RuntimeValue::String("HI".into())];
        assert_eq!(string_to_lower(&args).unwrap(), RuntimeValue::String("hi".into()));
    }

    #[test]
    fn split() {
        let args = vec![RuntimeValue::String("a,b,c".into()), RuntimeValue::String(",".into())];
        assert_eq!(
    ...
    fn join() {
        ...
        assert_eq!(string_join(&args).unwrap(), RuntimeValue::String("a-b".into()));
    }

    #[test]
    fn slice() {
        let args = vec![RuntimeValue::String("hello".into()), RuntimeValue::Int(1), RuntimeValue::Int(3)];
        assert_eq!(string_slice(&args).unwrap(), RuntimeValue::String("el".into()));
    }

    #[test]
    fn slice_out_of_bounds() {
        let args = vec![RuntimeValue::String("hi".into()), RuntimeValue::Int(0), RuntimeValue::Int(5)];
