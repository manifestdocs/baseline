...
use crate::analysis::infer::{InferCtx, builtin_generic_schemas};
...
fn module_descriptions() -> HashMap<&'static str, &'static str> {
    ...
    m.insert(
        "Db",
        "Connect to SQLite and run queries with parameter binding.",
    );
    m.insert(
        "Env",
    ...
    m.insert(
        "Sqlite",
        "Connect to SQLite databases, execute statements, and run typed queries.",
    );
    m.insert(
        "Row",
...
fn effects_for_function(qualified_name: &str) -> Vec<String> {
    ...
    let module = qualified_name.split('.').next().unwrap_or(qualified_name);
...
fn known_functions() -> Vec<(
    ...
    vec![
        ...
        (
            ...
            Some(
                "Some(42) |> Option.ok_or(\"not found\")\n// => Ok(42)\nNone |> Option.ok_or(\"not found\")\n// => Err(\"not found\")",
            ),
        ),
        // -- Result --
        (
        ...
        (
            "Result",
            "ensure",
            "Return Ok(()) if condition is true, or Err(err) if false. Use with ? for guard clauses.",
            Some(
        ...
        (
            "Sqlite",
            "execute!",
            "Execute a SQL statement with parameter binding. Returns affected row count.",
            Some("Sqlite.execute!(\"INSERT INTO users (name) VALUES (?1)\", [name])"),
        ),
        (
            "Sqlite",
            "query!",
            "Run a SQL query and return all matching rows as typed Row values.",
            Some("let rows = Sqlite.query!(\"SELECT * FROM users WHERE active = ?1\", [\"1\"])"),
        ),
        (
            "Sqlite",
            "query_one!",
            "Run a SQL query and return the first row as Option<Row>.",
            Some("let user = Sqlite.query_one!(\"SELECT * FROM users WHERE id = ?1\", [id])"),
        ),
        (
            "Sqlite",
            "query_map!",
            "Run a SQL query and map each row with a function, returning a typed list.",
            Some(
        ...
        (
            "Row",
            "string",
            "Extract a column value as a String.",
            Some("Row.string(row, \"name\")  // => \"Alice\""),
        ),
        (
            "Row",
            "int",
            "Extract a column value as an Int (no string parsing).",
            Some("Row.int(row, \"id\")  // => 42"),
        ),
        (
            "Row",
            "float",
            "Extract a column value as a Float.",
            Some("Row.float(row, \"price\")  // => 9.99"),
        ),
        (
            "Row",
            "bool",
            "Extract a column value as a Boolean (Int 1 = true, 0 = false).",
            Some("Row.bool(row, \"active\")  // => true"),
        ),
        (
            "Row",
            "optional_string",
            "Extract a column value as Option<String> (Null becomes None).",
...
fn build_type_signatures() -> HashMap<String, Type> {
    ...
    sigs.insert(
        "DateTime.diff".into(),
        Type::Function(
            vec![Type::Unknown, Type::Unknown, Type::String],
            Box::new(Type::Int),
        ),
    );
    ...
    sigs.insert(
        "Env.get!".into(),
        Type::Function(vec![Type::String], Box::new(option_string)),
    );
    sigs.insert(
        "Env.set!".into(),
    ...
    // HttpError
    sigs.insert(
        "HttpError.bad_request".into(),
        Type::Function(vec![Type::String], Box::new(Type::Unknown)),
    );
    sigs.insert(
        "HttpError.not_found".into(),
        Type::Function(vec![Type::String], Box::new(Type::Unknown)),
    );
    sigs.insert(
    ...
    sigs.insert(
        "HttpError.forbidden".into(),
        Type::Function(vec![Type::String], Box::new(Type::Unknown)),
    );
    sigs.insert(
        "HttpError.conflict".into(),
        Type::Function(vec![Type::String], Box::new(Type::Unknown)),
    );
    sigs.insert(
        "HttpError.unprocessable".into(),
        Type::Function(vec![Type::String], Box::new(Type::Unknown)),
    );
    sigs.insert(
    ...
    sigs.insert(
        "HttpError.method_not_allowed".into(),
        Type::Function(vec![Type::String], Box::new(Type::Unknown)),
    );
    sigs.insert(
        "HttpError.too_many_requests".into(),
        Type::Function(vec![Type::String], Box::new(Type::Unknown)),
    );
    sigs.insert(
        "HttpError.bad_gateway".into(),
        Type::Function(vec![Type::String], Box::new(Type::Unknown)),
    );
    sigs.insert(
        "HttpError.service_unavailable".into(),
        Type::Function(vec![Type::String], Box::new(Type::Unknown)),
    );
    sigs.insert(
        "HttpError.gateway_timeout".into(),
        Type::Function(vec![Type::String], Box::new(Type::Unknown)),
    );
    ...
    sigs.insert(
        ...
        Type::Function(
            vec![Type::String, Type::List(Box::new(Type::String))],
            Box::new(query_row_ty),
        ),
    );
    sigs.insert(
        "Sqlite.query_one!".into(),
        Type::Function(
            vec![Type::String, Type::List(Box::new(Type::String))],
            Box::new(option_row),
        ),
...
fn extract_var_names(qualified_name: &str, type_params: u32) -> HashMap<u32, String> {
    ...
    let param_names: Vec<&str> = if qualified_name.starts_with("Map.") {
        vec!["K", "V", "T", "U"]
    } else if qualified_name.starts_with("Result.") || qualified_name.contains("Result") {
        match type_params {
            2 => vec!["T", "E"],
            3 => vec!["T", "U", "E"],
            _ => vec!["T", "E", "U", "V"],
    ...
    match ty {
        ...
        Type::Enum(name, variants) => match name.as_str() {
            "Option" => {
                ...
                {
                    return format!("Option<{}>", format_type_with_vars(inner, var_names));
                }
                "Option".to_string()
            }
            "Result" => {
...
pub fn generate_docs() -> DocsOutput {
    ...
    let modules_vec: Vec<ModuleDoc> = modules
        .into_iter()
        .map(|(name, functions)| {
            let description = mod_descs.get(name.as_str()).unwrap_or(&"").to_string();
...
mod tests {
    ...
    fn test_generate_docs_has_modules_and_functions() {
        ...
        assert!(
            total_fns > 50,
            "should have many functions, got {}",
            total_fns
        );
    }
    ...
    fn test_markdown_includes_examples() {
        ...
        assert!(
            md.contains("**Example:**"),
            "markdown should include examples"
        );
