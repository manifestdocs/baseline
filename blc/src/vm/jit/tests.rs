...
use crate::vm::ir::{BinOp, Expr, IrFunction, IrModule, MatchArm, Pattern, Span, TagRegistry, UnaryOp};
use crate::vm::nvalue::{HeapObject, NValue};
    ...
    fn jit_sra_escape_falls_back() {
        ...
        assert!(candidates.is_empty(), "Escaping record should not be SRA candidate");
    }
    ...
    fn optimize_compile_and_run(module: &mut IrModule) -> NValue {
        ...
        let program = compile(module, false).expect("JIT compilation failed");
        program.run_entry_nvalue().expect("Entry function not compiled")
    }
    ...
    fn jit_list_concat() {
        ...
        let mut module = IrModule {
            functions: vec![IrFunction {
                ...
                body: Expr::BinOp {
                    op: BinOp::ListConcat,
                    lhs: Box::new(Expr::MakeList(
                        vec![make_int(1), make_int(2)],
                        None,
                    )),
                    rhs: Box::new(Expr::MakeList(
